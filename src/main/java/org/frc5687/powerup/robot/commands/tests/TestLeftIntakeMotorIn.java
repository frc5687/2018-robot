...
public class TestLeftIntakeMotorIn extends Command {
    private Intake _intake;
    private PDP _pdp;
    private long _timeout = 1000;
    private long _endtime;

    public TestLeftIntakeMotorIn(Intake intake, PDP pdp) {
        _intake = intake;
        _pdp = pdp;
    }

    public TestLeftIntakeMotorIn(Intake intake, PDP pdp, long timeout) {
        _intake = intake;
        _pdp = pdp;
        _timeout = timeout;
    }
    ...
    protected void initialize() {
        ...
        _intake.drive(Constants.Intake.TEST_INTAKE_SPEED, 0);
        DriverStation.reportError("Running left intake in for one second", false);
    }
